...
export const ResizeHandler = GObject.registerClass({
    ...
    _init(extension) {
        ...
        this._suppressedMaximizedResizes = new WeakSet();
    ...
    onResizeBegin(window, grabpo) {
        if (window.is_maximized()) {
            this._suppressedMaximizedResizes.add(window);
            Logger.log(`[MAXIMIZED] Suppressing resize tracking for active maximized ${window.get_id()}`);
            return;
        }
    ...
    onResizeEnd(window, grabpo, skipTiling) {
        if (this._finishSuppressedMaximizedResize(window)) return;
        this._finishTrackedResize(window, grabpo, skipTiling);
    }

    _finishSuppressedMaximizedResize(window) {
        if (!this._suppressedMaximizedResizes.delete(window)) return false;
        Logger.log(`[MAXIMIZED] Resize grab ended for ${window.get_id()}; reasserting maximized layout`);
        const workspace = window.get_workspace?.();
        const monitor = window.get_monitor?.();
    ...
    _finishTrackedResize(window, grabpo, skipTiling) {
    ...
    onSizeChange = (_, win, mode) => {
        ...
        if (mode === Meta.SizeChange.MAXIMIZE) {
            ...
            this.tilingManager.maximizedLayout.nativeStateChanged(window);
        }
    };
    ...
    onSizeChanged = (_, win) => {
        ...
        if (this._consumeRoleOwnedSizeChange(window, roleOwnedAtEntry)) return;
    ...
    _updatePreferredSizeFromResize(window, rect, { isConstrained, userForcedResize, isMonitorSized, clientOwnedSize }) {
        if (WindowState.get(window, 'targetRestoredSize') && !userForcedResize) return;
