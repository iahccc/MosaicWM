...
export const DrawingManager = GObject.registerClass({
    ...
    showFocusCorners(rect) {
        // A newly mapped window can still have an invalid transform. Passing
        // NaN to set_position poisons Clutter's allocation even after later moves.
        if (!rect || ![rect.x, rect.y, rect.width, rect.height].every(Number.isFinite)) {
            this.hideFocusCorners();
            return;
        }

        const previous = this._focusCornersRect;
        if (previous && previous.x === rect.x && previous.y === rect.y &&
            previous.width === rect.width && previous.height === rect.height)
            return;
        this._focusCornersRect = { ...rect };
