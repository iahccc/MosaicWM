...
export const MiniatureLayoutProfile = Object.freeze({
    NORMAL: 'normal',
    MAXIMIZED_FOCUSED: 'maximized-focused',
});
...
export function miniatureTargetSize(profile, defaultSize) {
    const divisor = profile === MiniatureLayoutProfile.MAXIMIZED_FOCUSED ? 2 : 1;
...
export function solveMaximizedRail({
    ...
    const sides = requiredSide ? [requiredSide] : MAXIMIZED_SIDE_ORDER;
    const candidates = sides.map(side => buildMaximizedCandidate(
...
function chooseBestCandidate(candidates, previousSide, sideOrder = SIDE_ORDER) {
    let best = null;
