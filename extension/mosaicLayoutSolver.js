...
export function solveMaximizedRail({
    ...
    // Preserve spatial continuity across dominant-window switches. Re-scoring all four
    // sides from scratch makes the rail jump (for example LEFT -> BOTTOM) when the newly
    // miniaturized dominant window changes the rail aspect ratio by even a few pixels.
    // As long as the currently committed side is still feasible, keep it. Only choose a
    // new side when that side can no longer satisfy the layout constraints.
    const stable = candidates.find(candidate => candidate.side === previousSide);
    if (stable) return stable;
    return chooseBestCandidate(candidates, null, MAXIMIZED_SIDE_ORDER);
}
