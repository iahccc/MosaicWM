...
export const AnimationsManager = GObject.registerClass({
    ...
    animateWindow(window, targetRect, options = {}) {
        ...
        if (this._applyWithoutMosaicAnimation(window, windowActor, targetRect,
            { userOp, firstPlacement, onComplete, draggedWindow })) {
            return;
        }
    ...
    _applyWithoutMosaicAnimation(window, windowActor, targetRect, options) {
        if (!this.shouldAnimateWindow(window, options.draggedWindow)) {
            this._applyWithoutAnimation(window, targetRect, options);
            return true;
        }
        if (!windowActor) {
            this._applyNoActor(window, targetRect, options);
            return true;
        }
        // Shell's native size-change animation already captured the visible source frame
        // and freezes the actor until the resized client buffer arrives. Only update its
        // destination here: move_frame() would release that freeze at an intermediate
        // position, then our scale ease would enlarge the still-maximized buffer again.
        if (windowActor.__animationInfo && !options.firstPlacement) {
            this._applyWithoutAnimation(window, targetRect, options);
            return true;
        }
        return false;
    ...
    _storeDeferredEntrance(window, windowActor, targetRect, easeParams) {
        ...
        this._armDeferredEntranceDeadline(window, pending);
    ...
    // Mapping, allocation and size-changed events can all fail to arrive when the client
    // never commits the planned configure (a natively maximized window ignores
    // move_resize_frame, a hung client stops answering). The visibility transaction must
    // still end: run the real entrance one last time, and reveal unconditionally if even
    // that gate stays closed. Without this the actor can stay at opacity=0 forever while
    // the windowHandler failsafe keeps re-arming on account of this pending entrance.
    _armDeferredEntranceDeadline(window, pending) {
        if (!this._timeoutRegistry) return;
        pending.deadlineId = this._timeoutRegistry.add(constants.SLIDE_IN_FAILSAFE_MS, () => {
            pending.deadlineId = 0;
            if (this._pendingEntranceEases.get(window.get_id()) !== pending) return GLib.SOURCE_REMOVE;
            if (!this.runDeferredEntrance(window))
                this._forceReleaseDeferredEntrance(window);
            return GLib.SOURCE_REMOVE;
        }, 'animations_deferredEntranceDeadline');
    }

    _forceReleaseDeferredEntrance(window) {
        const pending = this._clearDeferredEntrance(window.get_id());
        if (!pending) return;
        Logger.warn(`[ANIM] First placement ${window.get_id()} never reached its planned geometry; revealing without entrance`);
        WindowState.remove(window, 'pendingFirstPlacement');
        const actor = pending.windowActor ?? window.get_compositor_private();
    ...
    _clearDeferredEntrance(windowId) {
        ...
        if (pending.deadlineId) {
            this._timeoutRegistry?.remove(pending.deadlineId);
            pending.deadlineId = 0;
        }
    ...
    // A role transition (maximized/miniature/fullscreen handoff) supersedes ordinary Mosaic
    ...
    claimWindowForRoleTransition(window) {
        this._claimWindowPresentation(window, {preserveMiniatureTransform: true});
    }

    // Moving a MetaWindow between workspaces also changes who owns its presentation. Any
    // ease computed for the old workspace becomes invalid immediately; if it is allowed to
    // finish later it can leave a perfectly tiled MetaWindow visually translated or hidden.
    // A genuine first placement keeps its opacity gate so the destination can restart that
    // entrance from a fresh target. Existing windows are made fully visible synchronously.
    claimWindowForWorkspaceTransition(window) {
        this._claimWindowPresentation(window, {
            preserveFirstPlacement: !!WindowState.get(window, 'pendingFirstPlacement'),
            preserveMiniatureTransform: true,
        });
    }

    // Overflow admission is itself the membership transition. Once the window has been
    // rejected from its birth workspace, the original first-placement entrance can no longer
    // own visibility: its target belongs to the source workspace and later workspace signals
    // may otherwise preserve its opacity=0 gate indefinitely. Cancel that entrance here and
    // make the actor visible; the workspace switch plus destination retile now own presentation.
    claimWindowForOverflowTransition(window) {
        this._claimWindowPresentation(window, {
            preserveFirstPlacement: false,
            preserveMiniatureTransform: true,
        });
    }

    _claimWindowPresentation(window, {
        preserveFirstPlacement = false,
        preserveMiniatureTransform = false,
    } = {}) {
        if (!window) return;
        const id = window.get_id();
        ...
        this._clearDeferredEntrance(id);
        this._resetClaimedPresentationActor(
            window, preserveFirstPlacement, preserveMiniatureTransform);

        if (!preserveFirstPlacement)
            WindowState.remove(window, 'pendingFirstPlacement');
        WindowState.set(window, 'isMosaicResizing', false);
        this._animatingTargets.delete(id);
    ...
    _resetClaimedPresentationActor(window, preserveFirstPlacement, preserveMiniatureTransform) {
        const actor = window.get_compositor_private();
        ...
        const keepMiniature = preserveMiniatureTransform &&
        ...
        actor.opacity = preserveFirstPlacement ? 0 : 255;
    }
