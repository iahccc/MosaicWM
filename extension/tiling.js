...
export const TilingManager = GObject.registerClass({
    ...
    _drawTile(workspace, monitor, tile_info, meta_windows,
        dryRun = false, slotsOut = null, bounds = null) {
    ...
    _tileGeometryIsValid(tileInfo, bounds, label = 'layout') {
    ...
    _layoutRects(tileInfo, bounds) {
        if (!tileInfo?.levels) return [];
        return tileInfo.vertical
            ? this._verticalLayoutRects(tileInfo, bounds)
            : this._horizontalLayoutRects(tileInfo, bounds);
    }
    ...
    _horizontalLayoutRects(tileInfo, bounds) {
        const rects = [];
        let y = tileInfo.y;
    ...
    _verticalLayoutRects(tileInfo, bounds) {
        const rects = [];
        let x = tileInfo.x;
        for (const level of tileInfo.levels) {
            let y = level.y;
    ...
    _animateTileLayout(workspace, monitor, tile_info, work_area, meta_windows,
        draggedWindow = null, slotsOut = null, bounds = null) {
    ...
    _placeHorizontalAnimated(tile_info, meta_windows, ctx) {
        ...
        for (const level of tile_info.levels) {
            ...
            for (const windowDesc of level.windows) {
                const targetX = windowDesc.targetX ?? x;
                const targetY = windowDesc.targetY ?? y;
    ...
    _executeTilePass(workspace, referenceMetaWindow, keepOversizedWindows, dryRun, isRecursive, ctx) {
        ...
        this._updateMiniatureDescriptors(workspace, monitor, meta_windows, windows);
        ...
        const resolved = this._resolveOverflowByMiniature(
            ...
            meta_windows, windows, workspace, monitor, tileArea, tile_info);
        ...
        // to do next.
        if (overflow || !this._tileGeometryIsValid(tile_info, tileArea, 'workspace tile'))
        ...
        this._positionTiledWindows(
            ...
            referenceMetaWindow, computedSlots, tileArea);
    ...
    // Ordinary layouts pack miniatures alongside normal windows. Refresh both committed
    // and pending miniature footprints for the current focus profile before solving once.
    _updateMiniatureDescriptors(workspace, monitor, metaWindows, windows) {
    ...
    _positionTiledWindows(workspace, monitor, tile_info, tileArea, meta_windows,
        reference_meta_window, computedSlots, work_area) {
...
class Level {
    ...
    draw_horizontal(workspace, monitor, meta_windows, y, masks, isDragging,
        drawingManager, dryRun = false, slotsOut = null, bounds = null) {
        let x = this.x;
        for(const window of this.windows) {
            const rawX = window.targetX ?? x;
            const rawY = window.targetY ?? y;
