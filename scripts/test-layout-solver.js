...
function testMaximizedRailKeepsFeasiblePreviousSide() {
    const workArea = {x: 0, y: 0, width: 500, height: 500};
    const items = [{id: 1, width: 200, height: 100}];
    const options = {
        workArea,
        canonicalItems: items,
        actualItems: items,
    ...
    assert(natural);
    assert.equal(natural.side, RailSide.BOTTOM,
        'without history the larger dominant area should still choose bottom');

    const stabilized = solveMaximizedRail({...options, previousSide: RailSide.RIGHT});
    assert(stabilized);
    assert.equal(stabilized.side, RailSide.RIGHT,
        'a feasible previous rail side must survive focus-driven rescoring');
    assert(area(stabilized.maximizedRect) < area(natural.maximizedRect),
        'the regression requires the stable side to be strictly worse by area');
    assertValidSlots(stabilized, workArea);
}
...
function main() {
    ...
    testMaximizedRailKeepsFeasiblePreviousSide();
