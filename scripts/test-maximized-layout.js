...
export async function run() {
    ...
    try {
        ...
        // A maximized window that was alone (constraint rect == work area) and then yielded to
        // a peer must still recover dominance when that peer closes: the new plan is
        // geometry-identical to the pre-yield constraint, so only the miniature role check
        // separates "already committed" from "needs a restore".
        max.maximize();
        await waitFor(() => max.is_maximized() && !mini(max), 'solo dominant setup must present the window');
        const soloPeer = await create(500, 300);
        await waitFor(() => mini(max) && !mini(soloPeer), 'solo dominant peer must push max into the rail');
        soloPeer.delete(global.get_current_time());
        await waitFor(() => !global.display.list_all_windows().includes(soloPeer), 'solo peer must close');
        await waitFor(() => !mini(max) && max.is_maximized(), 'closing the only peer must restore the solo dominant');
        max.unmaximize();
        await waitFor(() => !max.is_maximized(), 'solo dominant regression must unmaximize cleanly');
