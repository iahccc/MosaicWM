...
export async function run() {
    ...
    try {
        ...
        // A fresh profile can open its first window natively maximized (small/headless monitors,
        // or a restored session state). The gate below needs a client that can actually accept
        // the planned configure, so normalize the probe to a settled normal frame first.
        if (probe.is_maximized()) {
            probe.unmaximize();
            assert(await waitFor(() => !probe.is_maximized() &&
                !WindowState.get(probe, 'targetRestoredSize') &&
                !WindowState.get(probe, 'targetSmartResizeSize')),
            'Probe Firefox must return to a settled normal frame before entrance-gate setup');
        }
